...
bool ULedgeDetector::DetectLedge(FLedgeDetectionInfo& LedgeDetection)
{
	...
	if (HasClimbForward() && HasClimbTop() && HasCharacterFit())
	{
		...
		LedgeDetection.Location = DownwardCheckHitResult.ImpactPoint +
			CapsuleComponent -> GetScaledCapsuleHalfHeight() * FVector::UpVector
				+ CapsuleComponent -> GetScaledCapsuleRadius() / 2 * ForwardCheckHitResult.ImpactNormal;
		LedgeDetection.Rotation = (ForwardCheckHitResult.ImpactNormal * Rotation).Rotation();
...
bool ULedgeDetector::HasCharacterFit()
{
	...
	decltype(auto)  value = !World -> OverlapBlockingTestByProfile(OverlapLocation, FQuat::Identity, FName("Pawn"), OverlapCapsuleShape, QueryParams);
